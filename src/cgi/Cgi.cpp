...
void createEnv(std::map<std::string, std::string> const &metaVariables,
               std::vector<std::string> &tmpEnv, std::vector<char *> &envp) {
  {
...
Cgi::execute(std::string const &resourcePath, std::string const &bodyPath,
  ...
  // TODO: ce pas fermer ce fd et le passé a un orchestrateur qui va appelr la
  // methode d'ajoue a epoll du transport
  close(fds[1]);
}

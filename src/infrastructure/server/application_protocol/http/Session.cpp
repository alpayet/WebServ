...
Session::pushRequest(char const *inputBuf, std::size_t size, RequestStatus::Type status)
{
	return (_protocol.pushRequest(_context, inputBuf, size, status));
}

Session::PushStatus::Type
Session::pushStream(char const *streamBuf, std::size_t size, StreamStatus::Type status)
{
...
{
	app::StreamInfo const resources = {
		_context.stream.stream_info.fd, _context.stream.stream_info.pid
	};

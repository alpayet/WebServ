...
/*   Updated: 2026/08/15 13:16:24 by alpayet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Parser::Parser(
	...
	: _maxRequestLineSize(validationPolicy.getMaxRequestLineSize()),
	  ...
	  _maxBodySize(validationPolicy.getMaxBodySize()), _httpVersionProvider(httpVersionProvider)
{}
...
Parser::Step Parser::parse(std::vector<char> &inputBuf, State &state)
{
	...
	while (can_continue && !inputBuf.empty())
	{
		switch (state.step)
		{
			...
			case Parser::BODY:
			{
				std::size_t const before = state.bodyBytesRead;
				parseBody(inputBuf, state.request, state.bodyBytesRead);
				std::size_t const consumed = state.bodyBytesRead - before;
				inputBuf.erase(

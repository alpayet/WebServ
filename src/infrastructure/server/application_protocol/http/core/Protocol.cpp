...
Protocol::PushStatus::Type Protocol::pushStream(
	Context &context, std::vector<char> const &streamBuf, StreamStatus::Type status
)
{
	// TODO: voir avec luca si il faut keep alive le client si le cgi associé timeout ou si il eof
	// trop tot
	try
	{
...
void Protocol::dispatchCgiResponse(Context &context)
{
	...
	switch (context_stream.state.response.getType())
	{
		...
		case cgi::Response::CLIENT_REDIR:
			context_output.response = mapper::CgiResponse::toHttpResponse(context_stream.state.response);
			break;

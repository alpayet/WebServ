...
Protocol::Protocol(request::Parser &requestParser, cgi::Parser &cgiParser,
...
Protocol::PushStatus::Type Protocol::pushRequest(Context &context,
                                                 char const *inputBuf,
                                                 std::size_t size,
...
Protocol::PullStatus::Type
Protocol::pullResponse(Context &context, std::vector<char> &outputBuf) {
  try {
    Context::Output &context_output = context.output;

    if (_sender.produce(outputBuf, context_output.response,
                        context_output.reader,
...
void Protocol::dispatchCgiResponse(Context &context) {
  ...
  case cgi::Response::LOCAL_REDIR:
    if (++context_stream.localRedirDepth >
        Context::Stream::MAX_LOCAL_REDIR_DEPTH)
      throw Exception(Exception::MAX_LOCAL_REDIR_DEPTH_EXCEEDED);

    context_input.state.request.setTarget(
        context_stream.state.response.getLocationUri());
    context_input.state.request.setQuery(
        context_stream.state.response.getLocationQuery());
...
void Protocol::prepareDirectResponse(unsigned short statusCode,
                                     Response &response,
                                     app::IResourceReader **reader) {
  std::map<int, std::string> const &error_pages =
      _errorPagesProvider.getErrPages();
  ...
  if (it != error_pages.end()) {
    ...
    if (error_page_info.exists &&
        !(error_page_info.type == domain::COLLECTION) &&

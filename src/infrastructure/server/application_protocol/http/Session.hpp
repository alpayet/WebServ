...
class Session : public appProtocol::IProtocol
{
  public:
	...
	virtual PushStatus::Type
	pushRequest(char const *inputBuf, std::size_t size, RequestStatus::Type status);
	virtual PushStatus::Type
	pushStream(char const *streamBuf, std::size_t size, StreamStatus::Type status);

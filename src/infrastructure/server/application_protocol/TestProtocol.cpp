...
TestProtocol::TestProtocol(std::string const &body) : m_request(), m_response()
{
...
TestProtocol::pushRequest(std::vector<char> const &inputBuf, RequestStatus::Type status)
{
	// if (len == 0)
	//   return SEND_OK;
	//
...
IProtocol::PushStatus
TestProtocol::pushStream(std::vector<char> const &streamBuf, StreamStatus::Type status)
{
	return IProtocol::PUSH_COMPLETE;
}
...
TestProtocolFactory::TestProtocolFactory(std::string const &body) : m_body(body) {}

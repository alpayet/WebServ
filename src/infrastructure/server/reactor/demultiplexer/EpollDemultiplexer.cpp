...
EpollDemultiplexer::EpollDemultiplexer()
    : m_epoll_fd(epoll_create(1)), m_events() {
...
int EpollDemultiplexer::getEventFd(int const index) const {
  return m_events[index].data.fd;
}

bool EpollDemultiplexer::isReadable(int const index) const {
  return (m_events[index].events & EPOLLIN) != 0;
}

bool EpollDemultiplexer::isWritable(int const index) const {

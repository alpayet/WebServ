...
Location ServerConfig::findLocationFromUri(std::string const &uri) const {
  ...
  while (pos != std::string::npos) {
    ...
    for (std::vector<Location>::const_iterator it = this->m_locations.begin();
         it != ite; ++it) {
      if (it->path == (dir_path + "/"))
        return *it;
    }
...
ServerConfig::getAllowedMethods(Location const &loc) const {
...
std::string
ServerConfig::resolvePhysicalPath(std::string const &uri,
...
app::SystemResourceInfo setSRI(std::string const &path) {
  ...
  if (fileSystem::isReadable(sri.resourcePath))
    sri.permissions = static_cast<domain::ResourcePermissions>(
        sri.permissions | domain::readable);
  if (fileSystem::isWritable(sri.resourcePath))
    sri.permissions = static_cast<domain::ResourcePermissions>(
        sri.permissions | domain::writable);
  if (fileSystem::isExecutable(sri.resourcePath))
    sri.permissions = static_cast<domain::ResourcePermissions>(
        sri.permissions | domain::executable);
...
app::SystemResourceInfo
ServerConfig::locate(std::string const &id, std::string const &matchedRoute,
                     std::string const &rootPath) const {
  app::SystemResourceInfo sri;
...
app::SystemResourceInfo
ServerConfig::locateDefaultIndex(std::vector<std::string> const &indexesId,
                                 std::string const &matchedRoute,
                                 std::string const &rootPath) const {
  ...
  for (; it != ite; ++it) {
    if (fileSystem::exists(resPath + *it) &&
        fileSystem::isRegularFile(resPath + *it)) {
...
app::SystemResourceInfo
ServerConfig::locateErrorPage(std::string const &uri) const {
  app::SystemResourceInfo sri;
...
ServerConfig::ApplicativeProtocol
ServerConfig::getApplicativeProtocol(void) const {
...
std::ostream &operator<<(std::ostream &os, Location const &l) {
  ...
  }
  {
    ...
    for (std::vector<std::string>::const_iterator it = l.cgi.begin(); it != ite;
         ++it)
      os << "\tCGI: " << *it << std::endl;
  }
  os << "\tMethods: " << std::endl;
...
std::ostream &operator<<(std::ostream &os, ServerConfig const &s) {
  ...
  os << "\tRoot: " << s.getRoot() << std::endl;
  {
    ...
    for (std::vector<std::string>::const_iterator it = s.getIndex().begin();
         it != ite; ++it)
      os << "\tIndex: " << *it << std::endl;
  }
  os << "\tError pages: " << std::endl;
  {
    ...
    for (std::map<int, std::string>::const_iterator it = err_pages.begin();
         it != ite; ++it) {
  ...
  os << "\tClient max body size: " << s.getClientMaxBody() << std::endl;
  {
    ...
    for (std::vector<Location>::const_iterator it = locs.begin(); it != ite;
         ++it) {
      os << *it;
    }
  }

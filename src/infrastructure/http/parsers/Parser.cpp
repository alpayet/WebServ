...
namespace
{
	...
	bool	is_not_whitespaces(char c)
	{
		return (c != ' ' && c != '\t');
	}
...
namespace http
{
	...
	ParsingState::Step Parser::parse(
		std::vector<char> const &readBuf, ParsingState &state)
	{
		...
		std::vector<char>::const_iterator	it_line_end = std::search(it_start,
			readBuf.end(), _crlf, _crlf + _crlfSize);
		...
		std::vector<char>::const_iterator itLineEnd)
	{
		std::vector<char>::const_iterator	it_method_end = std::find_first_of(
			it, itLineEnd, _whiteSpaces, _whiteSpaces + _whiteSpaceSize);
	...
	std::string	Parser::extractTarget(std::vector<char>::const_iterator &it,
		std::vector<char>::const_iterator itLineEnd)
	{
		std::vector<char>::const_iterator	it_target_start = std::find_if(
			it, itLineEnd, is_not_whitespaces);
	...
	std::string	Parser::extractProtocol(std::vector<char>::const_iterator &it,
		std::vector<char>::const_iterator itLineEnd)
	{
		std::vector<char>::const_iterator	it_protocol_start = std::find_if(
			it, itLineEnd, is_not_whitespaces);
	...
	bool	Parser::hasLineBreak(std::vector<char>::const_iterator itStart,
		std::vector<char>::const_iterator itEnd)
	{
		return ((std::find_first_of(itStart, itEnd,
			_crlf, _crlf + _crlfSize) != itEnd));
	}

	void	Parser::skipLeadingCrlf(std::vector<char>::const_iterator &itStart,
		std::vector<char> const &readBuf, ParsingState &state)
	{

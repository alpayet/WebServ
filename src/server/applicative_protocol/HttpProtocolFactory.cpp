...
HttpProtocolFactory::HttpProtocolFactory(const ::ServerConfig &config)
    : _storage(), _directory_explorer(), _cgi(),
      _serveStaticResourceUseCase(config, _storage, _directory_explorer),
      _deleteStaticResourceUseCase(config, _storage),
      _executeDynamicResourceUseCase(config, _cgi),
      ...
      _router(config, _serveStaticResourceController,
